 ...
 */
	for (i = PART; i <= REGION; i++)
	{
		if (table & (1 << i))
		{
			...
			/*create simbolic link to /tmp */
			p = popen("pwd", "r");
			fscanf(p, "%s", pwd);
			/* if ORDER_LINE, create two links */
			if (i == ORDER_LINE)
			{
       		 		printf("creating links in /tmp to data file %s/%s\n", pwd, tdefs[ORDER].name);
				sprintf(cmd, "ln -fs %s/%s /tmp/%s", pwd, tdefs[ORDER].name, tdefs[ORDER].name);
				popen(cmd, "r");

       		 		printf("creating links in /tmp to data file %s/%s\n", pwd, tdefs[LINE].name);
				sprintf(cmd, "ln -fs %s/%s /tmp/%s", pwd, tdefs[LINE].name, tdefs[LINE].name);
				popen(cmd, "r");
			}
			/* if PART_PSUPP, create two links */
			else if (i == PART_PSUPP)
			{
       		 		printf("creating links in /tmp to data file %s/%s\n", pwd, tdefs[PART].name);
				sprintf(cmd, "ln -fs %s/%s /tmp/%s", pwd, tdefs[PART].name, tdefs[PART].name);
				popen(cmd, "r");

       		 		printf("creating links in /tmp to data file %s/%s\n", pwd, tdefs[PSUPP].name);
				sprintf(cmd, "ln -fs %s/%s /tmp/%s", pwd, tdefs[PSUPP].name, tdefs[PSUPP].name);
				popen(cmd, "r");
			}
			else
			{
       		 		printf("creating links in /tmp to data file %s/%s\n", pwd, tdefs[i].name);
				sprintf(cmd, "ln -fs %s/%s /tmp/%s", pwd, tdefs[i].name, tdefs[i].name);
				popen(cmd, "r");
			}
		}
	}

...
then
	...
	date
	cd $DBT3_INSTALL_PATH/scripts/pgsql
else
	echo "build the database without generating the data files"
fi
	echo "drop db"
	$DBT3_INSTALL_PATH/scripts/pgsql/drop_db.sh
	echo
	
	echo "create db"
	$DBT3_INSTALL_PATH/scripts/pgsql/create_db.sh
	echo
	
	echo "create tables"
	$DBT3_INSTALL_PATH/scripts/pgsql/create_tables.sh
	echo
	
	date
	echo "start loading db"
	$DBT3_INSTALL_PATH/scripts/pgsql/load_db.sh
	date
	...
	$DBT3_INSTALL_PATH/scripts/pgsql/create_indexes.sh
	date
	...
	$DBT3_INSTALL_PATH/scripts/pgsql/update_statistics.sh
	date
	echo "updating optimizer statistics done"
	
	date
	echo "start backup database"
	$DBT3_INSTALL_PATH/scripts/pgsql/backup_db.sh
	date
	echo "backup done"

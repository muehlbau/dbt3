...
my ($thruput_time);
$thruput_time = `psql -d $SID -c "select (e_time-s_time) as diff_time from time_statistics where task_name='PERF$perf_run_number.THRUPUT';"|grep -v row | grep -v diff`;
$thruput_time =~ s/-*//;
print "thruput_time is ", $thruput_time, "\n";
$thruput_time =~ s/\n//;
$thruput_time =~ s/^\s//;
chop($thruput_time);
